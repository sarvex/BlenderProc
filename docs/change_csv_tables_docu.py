...
class ConfigElement(object):
    ...
    def __repr__(self):
        if self.default_value:
            return f"{self.key_word}({self.ele_type}): {self.default_value}, description: {self.description}"
        else:
            return f"{self.key_word}({self.ele_type}), description: {self.description}"
                    ...
                    for line_nr, line in enumerate(lines[last_start_lines:]):
                        ...
                        if start_csv_table:
                            ...
                            if (
                                ...
                                and (
                                    space_pos > second_mark
                                    and (
                                        available != -1
                                        and space_pos < available
                                        or available == -1
                                    )
                                )
                            ):
                                if config_element := get_config_value_from_csv_line(
                                    line, current_line_nr
                                ):
                                    found_config_values.append(config_element)
                            elif "\"" in line and ":header" not in line:
